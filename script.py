...
vars = ['rflux', 'dt', 'jetmax_p', 'jetmax_m', 'njetmax', 'phimax', 'del_t']
...
for i in range(len(peaks60)):
    ...
    data[state]['beta6_a0']['jetmax_p'].append(rjet_max_p[peaks60[i]:troughs60[i]+1].max())
    data[state]['beta6_a0']['jetmax_m'].append(rjet_max_m[peaks60[i]:troughs60[i]+1].max())
...
for i in range(len(peaks69)):
    ...
    data[state]['beta6_a9']['jetmax_p'].append(rjet_max_p[peaks69[i]:troughs69[i]+1].max())
    data[state]['beta6_a9']['jetmax_m'].append(rjet_max_m[peaks69[i]:troughs69[i]+1].max())
...
for i in range(len(peaks29)):
    ...
    data['nonmad']['beta2_a9']['jetmax_p'].append(rjet_max_p[peaks29[i]:troughs29[i]+1].max())
    data['nonmad']['beta2_a9']['jetmax_m'].append(rjet_max_m[peaks29[i]:troughs29[i]+1].max())
...
for i in range(len(peaks20)):
    ...
    data['nonmad']['beta2_a0']['jetmax_p'].append(rjet_max_p[peaks20[i]:troughs20[i]+1].max())
    data['nonmad']['beta2_a0']['jetmax_m'].append(rjet_max_m[peaks20[i]:troughs20[i]+1].max())
...
def hist(data, xdata, ydata, state):
    labels = {
        ...
        'jetmax_p': 'Max Upper Jet Radius (in $r_G$)',
        'jetmax_m': 'Max Lower Jet Radius (in $r_G$)',
        'njetmax': 'Max Jet Efficiency (in %)',
        'phimax': 'Peak Flux'
    }
    bins = {
       'rflux': np.logspace(np.log10(0.025), np.log10(0.6), 26),
       'dt': np.logspace(np.log10(50), np.log10(700), 26),
       'jetmax_p': np.logspace(np.log10(2), np.log10(1500), 26),
       'jetmax_m': np.logspace(np.log10(1), np.log10(1500), 26),
       'njetmax': np.logspace(-1, 3, 26),
       'phimax': np.logspace(np.log10(40), np.log10(250), 26),
    }
    name = {
    ...
    title = {
        'mad': 'MAD Flux Eruptions',
        'nonmad': 'SANE Flux Eruptions',
        'all': 'MAD vs SANE Flux Eruptions'
    }
    clf()
    fig, ax = plt.subplots(2,2, figsize=(6,5), gridspec_kw=dict(height_ratios=[1,3], width_ratios=[3,1]))
    fig.delaxes(ax[0,1])
    if state in ['mad','nonmad']:
        for run in data[state].keys():
